    ...
    return (
        <>
            <div className="flex gap-3 relative">
                <button
                    onClick={handleShare}
                    className="p-3 rounded-full border border-zinc-700 text-zinc-400 hover:text-white hover:border-zinc-500 transition-colors group relative"
                    title={t('share')}
                >
                    {showCopiedToast ? <Check className="w-5 h-5 text-green-500" /> : <Share2 className="w-5 h-5" />}

                    {/* Tooltip/Toast for Copy Feedback */}
                    <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1 bg-zinc-800 text-white text-xs rounded-md whitespace-nowrap border border-zinc-700 pointer-events-none transition-all duration-200 ${showCopiedToast ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
                        {t('linkCopied')}
                    </div>
                </button>
