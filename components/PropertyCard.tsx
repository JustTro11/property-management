...
export default function PropertyCard({ property }: { property: Property }) {
  ...
  const statusColors = {
    ...
    rented: 'bg-red-500/10 text-red-500 border-red-500/20',
  ...
  return (
    <Link
      ...
      className="group relative bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden hover:border-zinc-700 transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-500/10 flex flex-col h-full"
    >
      {/* Image Container */}
      ...
      <div className="p-5 flex-grow">
        <h3 className="text-lg font-semibold leading-6 text-white group-hover:text-indigo-400 transition-colors">
          <span className="absolute inset-0" />
          {property.title}
        </h3>
        <p className="mt-1 text-2xl font-bold text-white">
          ${property.price.toLocaleString()}
          <span className="text-sm font-normal text-zinc-400 ml-1">{t('perMonth')}</span>
        </p>
        ...
        <div className="mt-6 flex items-center justify-between border-t border-zinc-800 pt-4 text-zinc-400">
